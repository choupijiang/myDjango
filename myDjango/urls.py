...
urlpatterns = [
    ...
    # url(r'^create_project$', CreateView.as_view(model=Project, template_name="create_project.html", success_url="index"), name='create_project'),
    url(r'^create_task$',  CreateView.as_view(model=Task, template_name="create_task.html", success_url ='index', fields=['title','description','importance','project']), name="create_task"),
    # url(r'^create_task$',Task_CreateView.as_view() , name="create_task"),
    url(r'^project_list$',Project_list.as_view(), name="project_list"),
    # url (r'^developer_list$', ListView.as_view(model=Developer, template_name="developer_list.html"), name="developer_list"),
    url (r'^developer_list$', Project_list.as_view(), name = "project_list"),
    # url (r'^task_detail_(?P<pk>\d+)$', DetailView.as_view(model=Task, template_name="task_detail.html"), name="task_detail"),
    ...
    # url (r'^update_task_(?P<pk>\d+)$',UpdateView.as_view(model=Task, template_name="update_task.html", success_url="index", fields=["title", "description", "importance", "project"]), name="update_task"),
    url (r'^update_task_time_(?P<pk>\d+)$', Task_update_time.as_view(), name = "update_task_time"), url(r'task_delete_(?P<pk>\d+)$', Task_delete.as_view(), name="task_delete"),
    url (r'^update_task_(?P<pk>\d+)$', UpdateViewCustom.as_view(model=Task,url_name="update_task", success_url="public_index", fields=["title", "description", "importance", "project"] ), name="update_task"),
